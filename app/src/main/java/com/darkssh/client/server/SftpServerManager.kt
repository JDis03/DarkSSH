 ...
 */
class SftpServerManager(private val context: Context) {
    ...
    suspend fun start(
        ...
        try {
            ...
            sshServer = SshServer.setUpDefaultServer().apply {
                ...
                signatureFactories = BuiltinSignatures.VALUES
                    .filter { it.isSupported }
                    .toList() as List<NamedFactory<Signature>>
                
                @Suppress("UNCHECKED_CAST")
                cipherFactories = BuiltinCiphers.VALUES
                    .filter { it.isSupported }
                ...
                keyExchangeFactories = org.apache.sshd.server.ServerBuilder.setUpDefaultKeyExchanges(true)
                
                @Suppress("UNCHECKED_CAST")
                macFactories = BuiltinMacs.VALUES
                    .filter { it.isSupported }
                    .toList() as List<NamedFactory<Mac>>
                
                @Suppress("UNCHECKED_CAST")
                compressionFactories = BuiltinCompressions.VALUES
                    .filter { it.isSupported }
                    .toList() as List<NamedFactory<Compression>>
                
                Timber.i("Configured factories - Signatures: ${signatureFactories.size}, KEX: ${keyExchangeFactories.size}, Ciphers: ${cipherFactories.size}, MACs: ${macFactories.size}")
     ...
     */
    internal fun broadcastUploadComplete(path: String, size: Long) {
        try {
            val intent = Intent(ACTION_FILE_UPLOADED).apply {
